...
import com.durand.data.mapper.advance_vaccination.AdvanceVaccinationMapper
...
class AdvanceVaccinationRepositoryImpl : AdvanceVaccinationRepository {
    val gson = Gson()
    override suspend fun getAdvanceVaccination(params: String): ResultType<Failure, AdvanceVaccinationMasterResponseModel> {
        val query = "/vacunapp/api/persona_campana"
        ...
        return try {
            ...
            return if (result.isSuccessful) {
                val gson = Gson()
                val refVar = gson.fromJson(gson.toJson(result.body), Array<AdvanceVaccinationResponseEntities>::class.java)
                val professionsMapper = AdvanceVaccinationMapper.transform(AdvanceVaccinationMasterResponseEntities(refVar))
