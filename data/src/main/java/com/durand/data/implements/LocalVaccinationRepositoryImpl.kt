...
class LocalVaccinationRepositoryImpl : LocalVaccinationRepository {

    val gson = Gson()

    override suspend fun getLocalVaccination(params: String): ResultType<Failure, LocalVaccinationMasterResponseModel> {
        val query = "/vacunapp/api/local_vacunacion"
        ...
        return try {
            ...
            return if (result.isSuccessful) {
                ...
                val refVar = gson.fromJson(gson.toJson(result.body),
                        Array<LocalVaccinationResponseEntities>::class.java)
                val professionsMapper = LocalVaccinationMapper.transform(LocalVaccinationMasterResponseEntities(refVar))
